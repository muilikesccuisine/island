...
public class TaskController {
    ...
    @PostMapping("/publish")
    public Mono<Result<Task>> publish(@RequestBody TaskPublishReq req, @RequestHeader("X-User-Id") Long operatorId) {
        return taskService.publishTask(req, operatorId)
                .map(Result::success);
    }

    @PostMapping("/{taskId}/enroll")
    public Mono<Result<Void>> enroll(@PathVariable Long taskId, @RequestHeader("X-User-Id") Long survivorId) {
        return taskService.enrollTask(taskId, survivorId)
                .thenReturn(Result.success());
    }

    @PostMapping("/submit")
    public Mono<Result<Void>> submit(@RequestBody TaskSubmitReq req, @RequestHeader("X-User-Id") Long survivorId) {
        return taskService.submitTask(req, survivorId)
                .thenReturn(Result.success());
    }

    @PostMapping("/audit")
    public Mono<Result<Void>> audit(@RequestBody TaskAuditReq req, @RequestHeader("X-User-Id") Long operatorId) {
